...
export class KeysGenerator {
	...
	static async generateKeys() {
		...
		let privateKeyB64 = this.ABtoB64(await window.crypto.subtle.exportKey('pkcs8', privateKey))
		let publicKeyB64 = this.ABtoB64(await window.crypto.subtle.exportKey('spki', publicKey))

		return { publicKey: publicKeyB64, privateKey: privateKeyB64 }
	}

	static async importKey(key, type = 'private') {
		let extractedKey = await window.crypto.subtle.importKey(
