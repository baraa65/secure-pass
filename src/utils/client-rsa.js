...
export class ClientRSA {
	...
	async enc(m) {
		...
		let ciphertext = await window.crypto.subtle.encrypt({ name: 'RSA-OAEP' }, publicKey, encoded)
	...
	async dec(enc) {
		...
		let decrypted = await window.crypto.subtle.decrypt({ name: 'RSA-OAEP' }, privateKey, KeysGenerator.B64toAB(enc))
...
export const clientRSA = new ClientRSA()
